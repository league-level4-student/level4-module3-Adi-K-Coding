...
class ExceptionsTest {
	...
	public void testDivideByZeroException() {
		try {
			assertEquals(0, ExceptionMethods.divide(5.0, 0.0));
			fail("IllegalArguementException not thrown");
		} catch (IllegalArgumentException e) {
			e.printStackTrace();
			assertEquals(4.0, ExceptionMethods.divide(12.0, 3.0));
		}
	}
	...
	public void testReverseString() {
		try {
			assertEquals("dfd", ExceptionMethods.reverseString(""));
			fail("IllegalStateException not thrown");
		} catch (IllegalStateException e) {
			e.printStackTrace();
			assertEquals("esrever", ExceptionMethods.reverseString("reverse"));
		}
	}
